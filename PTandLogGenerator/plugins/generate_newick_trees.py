...
ROOT_DIR = os.path.dirname(os.path.abspath(__file__)) # Root of this file

sys.path.insert(0, '{}/../newick/'.format(ROOT_DIR))
sys.path.insert(0, '{}/../simpy/'.format(ROOT_DIR))
sys.path.insert(0, '{}/../source/'.format(ROOT_DIR))
...
def generate_tree(parameter_line,i,pop_index,render):
    ...
    tree_name = "{}/../data/trees/tree_".format(ROOT_DIR) + str(pop_index) + "_" + str(i)
    fname = tree_name + ".nw"
    ...
    if render == True:
        graphiv_converter = GraphvizTree(random_tree.t.write(format=1, format_root_node=True), pop_index, i, ROOT_DIR)
    return 1
...
def abortable_generate_tree(*args):
    ...
    try:
        ...
        print('aborted: tree_{}_{};{}'.format(str(args[2]), str(args[1]), str(args[-1])))
        p.terminate()
        return 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('input', help='give the csv-formatted file in which the' \
                                   ...
                                   '{}/../data/parameter_files/example_parameters.csv'.format(ROOT_DIR),
                        metavar='input_file')
    ...
    print("start of plugin with arguments: {}".format(args))
    ...
    for line in parameter_lines:
        if first_line:
            ...
            actual_parameter_order = parameters
            reorder_indexes = [desired_parameter_order.index(el) for el in actual_parameter_order]
            continue
        else:
            line = line.rstrip()
            parameters = line.split(';')
            parameters = [param for (index, param) in sorted(zip(reorder_indexes, parameters))]
        ...
        multiple_results = [pool.apply_async(abortable_generate_tree, args=(line, i,
                                                                            population_index,
                                                                            render_image,
                                                                            timeout)) for i in range(1, no_trees + 1)]
        print('generated {} models for population {}'.format(sum([res.get() for res in multiple_results]), str(population_index)))

        population_index += 1

    # print total generation time
    total_time = str(time.time()-start_time)
    print('total tree generation time: {}'.format(total_time))