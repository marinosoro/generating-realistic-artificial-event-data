...
def write_as_csv(traces,tree_index,log_index,record_timestamps):
    '''writes log to a csv-formatted file:
        case_id,act_name
        1,a
        1,b'''
    csv_file = open("{}/../data/logs/log".format(ROOT_DIR) + "{}_{}".format(tree_index, log_index) + ".csv", 'w')
...
def write_as_xes(traces,tree_index,log_index,record_timestamps):
    '''writes log to xes-formatted file'''
    xes_file = open("{}/../data/logs/log".format(ROOT_DIR) + "{}_{}".format(tree_index, log_index) + ".xes", 'w')
...
parser.add_argument('--l', nargs='?', default='1',
                    help='indicate how many logs to generate per tree ' \
                         'default=1', metavar='no_logs_per_tree')
...
for filepath in tree_files:
    ...
    for log_index in range(no_logs_per_tree):
        #generate traces
        ...
        if format_log == 'csv':
            write_as_csv(traces,tree_index+1,log_index+1,record_timestamps)
        elif format_log == 'xes':
            write_as_xes(traces,tree_index+1,log_index+1,record_timestamps)
